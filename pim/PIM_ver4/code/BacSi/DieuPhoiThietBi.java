...
public class DieuPhoiThietBi implements ActionListener, KeyListener{
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (e.getSource() == buttonThem) {
			MyHome.turnNext("DieuPhoiThietBi_Them", null);
		}
		else if (e.getSource() == buttonCapNhat) {
			MyHome.turnNext("DieuPhoiThietBi_CapNhat", table);
		}
...
public void setInformation() { 
		...
		try {
			...
			CallableStatement statement = connection.prepareCall("{ call rootdata.proc_dieuphoithietbi_laydieuphoithietbi_theobacsi (?,?) }");
			...
			data.setColumnIdentifiers(new String[] {"Ma Ca","Ma Thiet Bi","So Luong","Dieu Phoi","Ket Thuc"});
			data.setRowCount(0);
		...
		if ((table.getRowCount() > 0 && !textfieldChiTietTraCuu.getText().equals("")) || table.getSelectedRowCount() > 0) {
			if (table.getSelectedRowCount() > 0){
                ...
                for (int i = 0; i < rowCount; i++){
                    try {
    					Connection connection = MyHome.getDatabase().getConnection();
    					CallableStatement statement = connection.prepareCall("{ call rootdata.proc_dieuphoithietbi_xoa1dieuphoithietbi (?,?,?,?) }");
    					statement.setString(1,(String) table.getValueAt(selectedRows[i], mapping[0]));
    					statement.setString(2,(String) table.getValueAt(selectedRows[i], mapping[1]));
    					statement.setString(3,(String) table.getValueAt(selectedRows[i], mapping[3]));
    					statement.registerOutParameter(4,Types.INTEGER);
    					
    					statement.execute();
    					changedrows +=(int) statement.getObject(4);
    					statement.close();
    				} catch (SQLException e) {
    					JOptionPane.showMessageDialog(null, "Loi: " + e.getMessage(), "Loi", JOptionPane.ERROR_MESSAGE);
    					e.printStackTrace();
                ...
                for (int i = 0; i < table.getRowCount(); i++) {
                	try {
    					...
    					statement.setString(2,(String) table.getValueAt(i, mapping[1]));
    					statement.setString(3,(String) table.getValueAt(i, mapping[3]));
    					statement.registerOutParameter(4,Types.INTEGER);
