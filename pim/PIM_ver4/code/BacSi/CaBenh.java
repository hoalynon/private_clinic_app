...
public class CaBenh implements ActionListener, KeyListener{
	...
	private ButtonRound buttonKeThuoc;
	
	private TextFieldRound textfieldChiTietTraCuu;

	private JTable table;
	
	CaBenh(BSHome MyHome){
		...
		JLabel labelHeading = new JLabel("DANH SACH CA BENH");
		...
		buttonKeThuoc = new ButtonRound();
		buttonKeThuoc.setText("  Ke Thuoc  ");
		...
		buttonKeThuoc.setFont(new Font("Bevan", Font.BOLD, 12));
		buttonKeThuoc.setBorderPainted(false);
		panel2.add(buttonKeThuoc);
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (e.getSource() == buttonCapNhat) {
			MyHome.turnNext("CaBenh_CapNhat", table);
		}
		
		if (e.getSource() == buttonKeThuoc) {
			if (table != null && table.getSelectedRowCount() == 1) {
				MyHome.turnNext("CaBenh_KeThuoc", table);
			}
		}
	}
...
public void setInformation() { 
		...
		try {
			...
			CallableStatement statement = connection.prepareCall("{ call rootdata.proc_cabenh_laycabenh_theobacsi (?,?) }");
			...
			data.setColumnIdentifiers(new String[] {"Ma Ca","Ma BN","Ma Benh","Muc Do","Hinh Thuc","Bat Dau","Ket Thuc","Tinh Trang","Ma Phong"});
			data.setRowCount(0);
