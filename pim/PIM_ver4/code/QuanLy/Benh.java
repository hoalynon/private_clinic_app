...
public class Benh implements ActionListener, KeyListener{
	...
	Benh(QLHome MyHome){
		...
		JLabel labelHeading = new JLabel("DANH SACH BENH");
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (e.getSource() == buttonThem) {
			MyHome.turnNext("Benh_Them", null);
		}
		else if (e.getSource() == buttonCapNhat) {
			MyHome.turnNext("Benh_CapNhat", table);
		}
...
public void setInformation() { 
		...
		try {
			...
			data.setColumnIdentifiers(new String[] {"Ma Benh","Ten Benh","Ten Khoa","Gia Dieu Tri"});
			data.setRowCount(0);
		...
		if ((table.getRowCount() > 0 && !textfieldChiTietTraCuu.getText().equals("")) || table.getSelectedRowCount() > 0) {
				if (table.getSelectedRowCount() > 0){
	                ...
	                for (int i = 0; i < rowCount; i++){
	                    try {
	    					Connection connection = MyHome.getDatabase().getConnection();
	    					CallableStatement statement = connection.prepareCall("{ call rootdata.proc_benh_xoa1benh (?,?) }");
