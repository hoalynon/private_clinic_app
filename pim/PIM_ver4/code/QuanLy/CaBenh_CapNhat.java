...
public class CaBenh_CapNhat implements ActionListener, KeyListener, MouseListener{
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (e.getSource() == comboBoxTinhTrang) {
			...
			if (comboBoxTinhTrang.getItemAt(comboBoxTinhTrang.getSelectedIndex()).equals("Da ket thuc") && TinhTrangTruoc != 5) {
				System.out.println(TinhTrangTruoc);
	...
	public void CapNhat() {
		...
		try {
			Connection connection = MyHome.getDatabase().getConnection();
			CallableStatement statement = connection.prepareCall("{ call rootdata.proc_cabenh_sua1cabenh (?,?,?,?,?,?,?) }");
			
			statement.setString(1,textfieldMaCa.getText());
			statement.setString(2,stringMucDo);
			statement.setString(3,stringHinhThuc);
			statement.setString(4,stringKetThuc);
			statement.setString(5,stringTinhTrang);
			statement.setString(6,textfieldMaPhong.getText());
