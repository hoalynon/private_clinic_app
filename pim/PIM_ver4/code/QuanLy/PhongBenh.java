...
public class PhongBenh implements ActionListener, KeyListener{
	...
	PhongBenh(QLHome MyHome){
		...
		JLabel labelHeading = new JLabel("DANH SACH PHONG BENH");
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (e.getSource() == buttonThem) {
			MyHome.turnNext("PhongBenh_Them", null);
		}
		else if (e.getSource() == buttonCapNhat) {
			MyHome.turnNext("PhongBenh_CapNhat", table);
		}
...
public void setInformation() { 
		...
		try {
			...
			data.setColumnIdentifiers(new String[] {"Ma Phong","Loai","Toa","Lau","Suc Chua","Con Trong","Gia/Ngay"});
			data.setRowCount(0);
		...
		if ((table.getRowCount() > 0 && !textfieldChiTietTraCuu.getText().equals("")) || table.getSelectedRowCount() > 0) {
				if (table.getSelectedRowCount() > 0){
	                ...
	                for (int i = 0; i < rowCount; i++){
	                    try {
	    					Connection connection = MyHome.getDatabase().getConnection();
	    					CallableStatement statement = connection.prepareCall("{ call rootdata.proc_phongbenh_xoa1phongbenh (?,?) }");
