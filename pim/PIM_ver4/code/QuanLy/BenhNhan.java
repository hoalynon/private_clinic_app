...
public class BenhNhan implements ActionListener, KeyListener{
	...
	BenhNhan(QLHome MyHome){
		...
		JLabel labelHeading = new JLabel("DANH SACH BENH NHAN");
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (e.getSource() == buttonThem) {
			MyHome.turnNext("BenhNhan_Them", null);
		}
		else if (e.getSource() == buttonCapNhat) {
			MyHome.turnNext("BenhNhan_CapNhat", table);
		}
...
public void setInformation() { 
		...
		try {
			...
			data.setColumnIdentifiers(new String[] {"Ma BN","Ho Ten","Gioi Tinh","Ngay Sinh","Que Quan","Noi O Hien Tai","Dat Lich"});
			data.setRowCount(0);
		...
		if ((table.getRowCount() > 0 && !textfieldChiTietTraCuu.getText().equals("")) || table.getSelectedRowCount() > 0) {
				if (table.getSelectedRowCount() > 0){
	                ...
	                for (int i = 0; i < rowCount; i++){
	                    try {
	    					Connection connection = MyHome.getDatabase().getConnection();
	    					CallableStatement statement = connection.prepareCall("{ call rootdata.proc_benhnhan_xoa1benhnhan (?,?) }");
