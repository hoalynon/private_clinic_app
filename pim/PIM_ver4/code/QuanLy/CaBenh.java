...
public class CaBenh implements ActionListener, KeyListener{
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (e.getSource() == buttonThem) {
			MyHome.turnNext("CaBenh_Them", null);
		}
...
public void setInformation() { 
		...
		try {
			...
			data.setColumnIdentifiers(new String[] {"Ma Ca","Ma BN","Ma BS","Ma Benh","Muc Do","Hinh Thuc","Bat Dau","Ket Thuc","Tinh Trang","Ma Phong"});
			data.setRowCount(0);
		...
		if ((table.getRowCount() > 0 && !textfieldChiTietTraCuu.getText().equals("")) || table.getSelectedRowCount() > 0) {
				if (table.getSelectedRowCount() > 0){
	                ...
	                for (int i = 0; i < rowCount; i++){
	                    try {
	    					Connection connection = MyHome.getDatabase().getConnection();
	    					CallableStatement statement = connection.prepareCall("{ call rootdata.proc_cabenh_xoa1cabenh (?,?) }");
