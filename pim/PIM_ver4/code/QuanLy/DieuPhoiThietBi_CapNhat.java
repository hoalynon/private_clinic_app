...
public class DieuPhoiThietBi_CapNhat implements ActionListener, KeyListener, MouseListener{
	...
	DieuPhoiThietBi_CapNhat(QLHome MyHome, JTable table){
	...
	public void CapNhat() {
		...
		try {
			Connection connection = MyHome.getDatabase().getConnection();
			CallableStatement statement = connection.prepareCall("{ call rootdata.proc_dieuphoithietbi_sua1dieuphoithietbi (?,?,?,?,?,?) }");
			
			statement.setString(1,textfieldMaCa.getText());
			statement.setString(2,textfieldMaThietBi.getText());
			statement.setString(3,textfieldSoLuong.getText());
