...
public class Thuoc implements ActionListener, KeyListener{
	...
	Thuoc(QLHome MyHome){
		...
		JLabel labelHeading = new JLabel("DANH SACH THUOC");
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (e.getSource() == buttonThem) {
			MyHome.turnNext("Thuoc_Them", null);
		}
		else if (e.getSource() == buttonCapNhat) {
			MyHome.turnNext("Thuoc_CapNhat", table);
		}
...
public void setInformation() { 
		...
		try {
			...
			data.setColumnIdentifiers(new String[] {"Ma Thuoc","Ten Thuoc","Cong Dung","So Luong Con Lai","Gia 1 Vien"});
			data.setRowCount(0);
		...
		if ((table.getRowCount() > 0 && !textfieldChiTietTraCuu.getText().equals("")) || table.getSelectedRowCount() > 0) {
				if (table.getSelectedRowCount() > 0){
	                ...
	                for (int i = 0; i < rowCount; i++){
	                    try {
	    					Connection connection = MyHome.getDatabase().getConnection();
	    					CallableStatement statement = connection.prepareCall("{ call rootdata.proc_thuoc_xoa1thuoc (?,?) }");
