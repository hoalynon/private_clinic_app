...
public class BacSi implements ActionListener, KeyListener{
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (e.getSource() == buttonThem) {
			MyHome.turnNext("BacSi_Them", null);
		}
		else if (e.getSource() == buttonCapNhat) {
			MyHome.turnNext("BacSi_CapNhat", table);
		}
		...
		if ((table.getRowCount() > 0 && !textfieldChiTietTraCuu.getText().equals("")) || table.getSelectedRowCount() > 0) {
	        	...
	        	if (table.getSelectedRowCount() > 0){
	                ...
	                for (int i = 0; i < rowCount; i++){
	                    try {
	    					Connection connection = MyHome.getDatabase().getConnection();
	    					CallableStatement statement = connection.prepareCall("{ call rootdata.proc_bacsi_xoa1bacsi (?,?) }");
